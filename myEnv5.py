...
class CustomEnv(gym.Env):
    ...
    def __init__(self, n_robots=3):
        ...
        # Adjust the observation space to match the expected dimensions
        # Assuming the resize dimensions should be (600, 1000, 3) instead of (3300, 5700, 3)
    ...
    def step(self, actions):
        ...
        # Keep individual rewards for each robot, maybe for monitoring or debugging
        self.individual_rewards = rewards  # Store the individual rewards
        
        # Aggregate rewards for the training process; here we sum them up
        aggregated_reward = sum(rewards)  # You can change this aggregation logic as needed
        
        observation = self.render()
        terminated = any(terminals)
        truncated = False
        info = {'individual_rewards': rewards, 'other_info': infos}
        print('done:', terminals, 'info:', infos)
        
        return observation, float(aggregated_reward), terminated, truncated, info
    ...
    def render(self, mode='human'):
        # Ensure the rendered image matches the observation space dimensions
        img = self.mobile_robot.render_pathPlanning1()
        img_resized = cv.resize(img, (1000, 600))  # Resize to match the observation space
        return img_resized
...
class MobileRobot(object):
    ...
    def step_pathPlanning1(self, actions):
        ...
        reached_goals = [False] * self.n_robots  # Keeps track of whether each robot reached its goal
        ...
        for k in range(self.n_robots):
            ...
            if self.reachGoal(new_position, k):
                reward = 0
                info = f'Robot {k} reached its goal'
                reached_goals[k] = True  # Mark that this robot reached its goal
                
            elif self.HitObstacle(new_position, self.grid_set[k]):
                reward = -3
                new_position = self.robots_pos[k].copy()
                info = f'Robot {k} hit obstacle'

            new_positions.append(new_position)
            rewards.append(reward)
            infos.append(info)

        # Check if both robots reached their goals to finish the game
        if all(reached_goals):
            terminals = [True] * self.n_robots  # All robots finish the game
            
            # Print the state when both robots have reached their goals
            print("State when both robots have reached their goals:")
            for k in range(self.n_robots):
                print(f"State of robot {k}:\n{self.state_set[k]}")
            print("\n")
        
        else:
            terminals = [False] * self.n_robots  # Keep the game running
